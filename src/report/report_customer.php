                        ...
                        <div class="table-responsive">
                            <table class="table table-border" id="dataTable" width="100%" cellspacing="0">
                                <thead>
                                    ...
                                    <tbody>
                                    <?php
                                        $get_payment = 'SELECT cus.customer_id,LOWER(CONCAT(cus.first_name,cus.last_name)) as nama_customer,
                                        LOWER(cus.email) as email,IF(cus.active = 1,"Aktif","Tidak Aktif") as status FROM customer as cus';
                                        $sql_result = mysqli_query($host,$get_payment);
                                        while($data = mysqli_fetch_assoc($sql_result)){
                                    ?>
                                    <tr>
                                        <td class="text-center"><?php echo $data['customer_id']; ?></td>
                                        <td class="text-center"><?php echo $data['nama_customer']; ?></td>
                                        <td class="text-center"><?php echo $data['email']; ?></td>
                                        <td class="text-center"><?php echo $data['status']; ?></td>
                                        <td class="text-center">
                                            <a href="detail/detail_customer.php?id=<?php echo $data['customer_id']; ?>" class="btn btn-info">Detail Customer</a>
                                        </td>
                                    </tr>
                                        <?php }?>
                                </tbody>
